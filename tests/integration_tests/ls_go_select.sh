...
python3 $SSPATH/scene_select/ard_scene_select.py  --config dsg547_dev.conf --products '["usgs_ls8c_level1_2"]' --workdir $scratch/  --pkgdir  $scratch/ --logdir $scratch/ --env $PWD/interim-prod-wagl.env --project u46 --walltime 02:30:00  --index-datacube-env index-test-odc.env  --interim-days-wait 5 #--run-ard
