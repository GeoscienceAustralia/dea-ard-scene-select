...
class TreeToSearchExprs(Transformer):
    ...
    def field_gt_expr(self, field, value):
        if not field.endswith("_version"):
            ...
            raise ValueError(
                f"Can only use > with software version "
...
def test_parser():
    ...
    # Wagl version tests
    assert parse_expressions('wagl_version in ["1.2.3", "3.4.5"]') == {
        "wagl_version": Range("1.2.3", "3.4.5")
    }
    assert parse_expressions('wagl_version < "1.2.3.dev4"') == {
        "wagl_version": Range(None, "1.2.3.dev4")
    }
